...
| --- | --- | --- | --- |
| A Word document (`.docx`) after editing one paragraph | A `.docx` is a ZIP of parts. `word/document.xml` is recompressed (every byte of that member differs) and the central directory is rewritten; `word/media/image1.png`, styles, fonts, and themes are byte-identical members. | `ooxml-v1` inflates every member and chunks the canonical (raw XML + media) stream, cutting at part boundaries and at text seams inside parts. | Every image and every untouched part: identical blocks. Inside the edited `document.xml`, everything up to the edited region and everything after it from the next re-synchronized cut — shared headers, footers, tables, and appendices dedup even *across different documents* that reuse them. Excel and PowerPoint behave identically: an edited sheet or slide invalidates only its own chunks. (Under `ooxml-ber-v1` the reuse is member-level only, as with `zip-v1` — the price of byte-exactness.) |
| The same document re-saved by a different tool | Same parts, but the tool may recompress every member with different deflate settings. | `ooxml-v1` canonicalizes both uploads to the same bytes. | Everything — the two uploads converge to identical canonical chunks, so the re-save stores nothing new. (Under `ooxml-ber-v1`: only members whose compressed bytes are identical.) |
| A PDF after a full rewrite by a different tool, or an Acrobat-style incremental save | A rewrite re-serializes objects but keeps compressed image/font/content streams verbatim; an incremental save appends a new section and touches nothing before it. | `pdf-v1` cuts at object boundaries and inside large streams by the hash. | Verbatim streams reproduce identical chunks wherever their objects moved; an incremental save reuses every chunk of the original bytes and adds only the appended section. |
...
| `zip-v1` | `application/zip`, `application/java-archive`, `application/epub+zip`, OpenDocument | ZIP containers: a forward-only walk over local headers, member bytes, data descriptors, the central directory and end records, with every size claim reconciled. Members are never inflated. | Candidates are **member boundaries** (each local header, the central directory). A large member always begins a chunk; small members attach backward to the preceding chunk; inside a large member the gear hash places ordinary cuts. An unchanged member (a JAR class, an ODF part) therefore yields identical chunks wherever it appears. |
| `ooxml-v1` | **Word/Excel/PowerPoint** (`.docx`/`.xlsx`/`.pptx`) | The **canonicalizing** Office profile: members are inflated and re-emitted deterministically (STORE, zeroed metadata, original order and names), and chunks concatenate to that canonical form — raw XML and media, with deflate no longer hiding shared content. A digitally signed package fails hard toward `ooxml-ber-v1`; a corrupt member (CRC/size contradiction) rejects the stream. | Every part begins a chunk from the 4 KiB document-unit minimum (an edited slide or sheet invalidates only itself); `.rels` and property parts attach backward to their owner; inside large parts the gear hash cuts at content-defined positions over the *inflated* bytes, so shared headers, footers, tables, and appendices dedup across document variants. |
| `ooxml-ber-v1` | policy-selected for the same three types (never probed, never a registry default) | The **byte-exact-reversible** Office profile: the same part-aware walk over the original bytes, nothing re-encoded — digital signatures survive and downloads are byte-identical. | Part-aligned boundaries at the 4 KiB document-unit minimum over the stored (still-compressed) members; an unchanged stored member reuses its chunks, but deflated members that were recompressed do not — the price of byte-exactness. |
| `pdf-v1` | `application/pdf` | PDF documents: the `%PDF-` header, indirect objects (stream payloads skipped by their direct `/Length`, or by scanning for `endstream`), classic xref tables, trailers, `startxref`, and any number of incremental-update sections. Nothing is decompressed. | Candidates are **object boundaries** (each body item start) from the 4 KiB document-unit minimum, with gear cuts inside large stream payloads. Verbatim image/font/content streams reproduce identical chunks across rewrites, and an incrementally-updated file reuses every chunk of its original bytes. |
...
Not yet a profile: archive member decomposition (looking *inside* ZIP/TAR
members, recursively) and semantic Office Open XML boundaries (part and
relationship structure beyond the package layer). Each is separate,
versioned profile work.

The table above is registry **V2** (`ProfileRegistry::V2`, the default);
`ProfileRegistry::V1` — where the Office types were `zip-v1` and PDF was
generic — remains frozen and selectable. The full essence list is available
from `ProfileRegistry::essences`. Moving a media type between profiles,
changing a mask, seed, or candidate rule, or adding a profile changes the
boundaries a stream produces and therefore lands as a new registry/profile
version, never as an in-place tweak.

### Canonical vs byte-exact for Office packages

`ooxml-v1` trades byte-exact round-tripping for reuse: chunks reproduce the
package's deterministic **canonical form** (a valid, semantically identical
package), not the original upload. Two uploads that differ only in
compressor settings converge to identical canonical bytes — and therefore
identical chunks. The cost: the original deflate bytes are gone, so anything
that verifies a signature or hash over the *exact* uploaded file will not
match. That is exactly the case `ooxml-ber-v1` exists for: the same
part-aware boundaries over the untouched bytes. The canonicalizing profile
enforces the split — a package containing `_xmlsignatures/` fails hard with
an error directing it to BER rather than silently invalidating the
signature. The `PackageObserver` tap (canonicalizing profile only) reports
member names, canonical offsets, and each member's inflated bytes during
the single streaming pass, so metadata extraction and derived artifacts
