...
impl<'a> Member<'a> {
    pub(super) const fn stored(name: &'a str, bytes: &'a [u8]) -> Self {
        Self {
            name,
            bytes,
            deflate: false,
    ...
    pub(super) const fn deflated(name: &'a str, bytes: &'a [u8]) -> Self {
        Self {
            name,
            bytes,
            deflate: true,
    ...
    pub(super) const fn framed(self, framing: Framing) -> Self {
        Self { framing, ..self }
    }
}
...
pub(super) fn archive(members: &[Member<'_>], options: Options<'_>) -> Vec<u8> {
    let mut out = Vec::new();
